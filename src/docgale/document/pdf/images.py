...
def get_load_images_timeout() -> int:
    """返回 PDF 批量渲染的超时秒数。"""
    return _positive_int_env("DOCGALE_PDF_RENDER_TIMEOUT", 300)


def get_load_images_threads() -> int:
    """返回 PDF 批量渲染允许使用的进程数。"""
    return _positive_int_env("DOCGALE_PDF_RENDER_THREADS", 3)
...
def _install_pdf_render_parent_exit_watcher() -> None:
    ...
    watcher = threading.Thread(
        ...
        name="docgale-pdf-render-parent-exit-watcher",
        daemon=True,
    )
    watcher.start()
