...
_DISPLAY_FORMULA_TOKENS = frozenset({"display", "katex-display", "math-display", "mathjax-display", "docvortex-math--block"})
...
def extract_formula(element: etree._Element) -> FormulaExtraction | None:
    ...
    if latex := _subtree_attribute(element, "data-docvortex-latex"):
        return FormulaExtraction(latex, display, "docvortex_latex")
    ...
    if "docvortex-math" in _class_tokens(element):
        latex = strip_formula_delimiters("".join(element.itertext()))
        if latex:
            return FormulaExtraction(latex, display, "docvortex_text")
