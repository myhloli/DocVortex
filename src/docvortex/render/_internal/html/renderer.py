...
class _HtmlRenderer:
    ...
    def render(self) -> str:
        ...
        article = (
            ...
            f'data-docvortex-html-version="{DOCVORTEX_HTML_VERSION}" data-render-mode="{self.mode.value}">\n{body}\n</article>'
        )
        ...
        self.has_mermaid = article_soup.select_one(".docvortex-flowchart") is not None
    ...
    def _render_block_content(self, planned: PlannedBlock) -> str:
        ...
        if isinstance(block, PageAuxTextBlock):
            ...
            attrs = [f'class="docvortex-page-aux docvortex-page-aux--{class_name}"']
            return f"<div {' '.join(attrs)}>{rendered.html}</div>"
    ...
    def _render_list(self, block: ListBlock) -> str:
        ...
        for child in block.content:
            if isinstance(child, ListBlock):
                ...
                if not nested:
                    continue
                if not items:
                    items.append({"content": "", "attrs": ['class="docvortex-list-item--orphan"'], "nested": []})
            ...
            if not rendered.html and not marker:
                items.append(
                    {
                        "content": "",
                        "attrs": ['class="docvortex-list-item--markerless"', *_wire_block_attributes(child)],
                        "nested": [],
                    }
                )
                continue
            ...
            if class_name == "docvortex-list--explicit":
                attrs.append('class="docvortex-list-item--explicit"')
            ...
            if class_name == "docvortex-list--explicit" and not marker_html:
        ...
        container_attrs = [f'class="docvortex-list {class_name}"', *_wire_block_attributes(block)]
        if list_type:
    ...
    def _render_image_body(self, parent: ImageBlock, block: ImageBodyBlock) -> str:
        ...
        parts = [self._render_image(source, alt=alt, class_name="docvortex-image")] if source else []
        if content.html:
            if source:
    ...
    def _render_table_body(self, block: TableBodyBlock) -> str:
        ...
        if content:
            ...
            if source_soup.find("table") is not None:
                ...
                source = self._safe_block_image_source(block)
                if source:
                    rendered_image = self._render_image(source, alt="table", class_name="docvortex-table-image")
                    return _wrap_visual_body(rendered_image, block, "table")
                return _wrap_visual_body(_render_raw_fallback(content), block, "table")
            rendered_text = f'<pre class="docvortex-table-text">{html.escape(normalized_content, quote=False)}</pre>'
            return _wrap_visual_body(rendered_text, block, "table")
        source = self._safe_block_image_source(block)
        rendered_image = self._render_image(source, alt="table", class_name="docvortex-table-image") if source else ""
        return _wrap_visual_body(rendered_image, block, "table")
    ...
    def _render_chart_body(self, parent: ChartBlock, block: ChartBodyBlock) -> str:
        ...
        parts = (
            [self._render_image(source, alt=parent.sub_type or "chart", class_name="docvortex-chart-image")] if source else []
        )
        if content.html:
            if source:
    ...
    def _render_code_body(self, parent: CodeBlock, block: CodeBodyBlock | AlgorithmBodyBlock) -> str:
        ...
        if parent.sub_type == BlockType.CODE:
            ...
            if not block.content:
                return _wrap_visual_body('<pre class="docvortex-code"><code></code></pre>', block, "code")
            language = _normalize_prism_language(parent.guess_lang)
            class_attr = f' class="language-{language}"' if language else ""
            pre_class = f"docvortex-code language-{language}" if language else "docvortex-code"
        ...
        if parent.sub_type == RAW_ALGORITHM:
            ...
            self._observe_inline(rendered)
            algorithm = f'<div class="docvortex-algorithm">{rendered.html}</div>' if rendered.html else ""
    ...
    def _render_standalone(self, article: str) -> str:
        ...
        return (
            ...
            f"{dependency_html}\n"
            "</head>\n"
            '<body class="docvortex-html-body">\n'
            f"{article}\n"
            "</body>\n"
            "</html>"
        )
...
document.addEventListener('DOMContentLoaded', function () {{
  var root = document.querySelector('.docvortex-document');
  var hosts = root ? Array.from(root.querySelectorAll('.docvortex-flowchart')) : [];
