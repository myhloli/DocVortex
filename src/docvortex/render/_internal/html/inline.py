...
def render_math_html(latex: str, *, display: bool) -> HtmlInlineResult:
    ...
    if display:
        return HtmlInlineResult(
            '<div class="docvortex-math docvortex-math--block" data-block-type="equation" '
            f'data-formula-display="block" data-docvortex-latex="{attribute_latex}">\\[\n{escaped}\n\\]</div>',
            has_math=True,
        )
    return HtmlInlineResult(
        '<span class="docvortex-math docvortex-math--inline" data-block-type="equation" '
        f'data-formula-display="inline" data-docvortex-latex="{attribute_latex}">\\({escaped}\\)</span>',
        has_math=True,
    )
    ...
    *,
    ...
    if isinstance(span, TextSpan):
        ...
        if span.styles and _needs_whitespace_preservation(span.content):
            styled_html = f'<span class="docvortex-preserve-whitespace">{styled_html}</span>'
...
def _apply_html_styles(content: str, styles: list[str]) -> str:
    ...
    if "emphasis" in styles:
        content = f'<span class="docvortex-text-emphasis">{content}</span>'
    return content
