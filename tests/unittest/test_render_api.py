...
import docvortex.render as render_module
...
from docvortex.schema import ImageBlock, ImageBodyBlock, MiddleJson, PageBlock, PageInfo, Producer, TextBlock
...
def test_unified_render_dispatches_all_native_output_types_without_mutation() -> None:
    ...
    epub = render(middle, RenderFormat.EPUB, options=EpubRenderOptions(modified_at=datetime(2026, 1, 2, tzinfo=timezone.utc)))
...
def test_unified_render_forwards_format_specific_options() -> None:
    ...
    image_middle = _middle(
        _page(
            ...
            ImageBlock(
                type="image",
                index=0,
                content=[ImageBodyBlock(type="image_body", index=0, content="", image_path="images/a b.png")],
            ),
        )
    )
    markdown = render(middle, RenderFormat.MARKDOWN, options=MarkdownRenderOptions(mode=RenderMode.FULL))
    html = render(middle, RenderFormat.HTML, options=HtmlRenderOptions(mode=RenderMode.FULL, standalone=False))
    ...
    image_markdown = render(
        image_middle, RenderFormat.MARKDOWN, options=MarkdownRenderOptions(asset_base_url="https://cdn.example/doc")
    )
    html_document = render(
        image_middle,
        RenderFormat.HTML,
        options=HtmlRenderOptions(asset_base_url="https://cdn.example/doc", document_title="Unified Render"),
    )
    latex_document = render(
        image_middle,
        RenderFormat.LATEX,
        options=LatexRenderOptions(asset_base_path="document assets", document_title="Unified Render"),
    )
    pdf = render(middle, RenderFormat.PDF, options=PdfRenderOptions(document_title="Unified Render"))
    ...
    assert "\\hypersetup{pdftitle={Unified Render}}" in latex_document
    assert "\\detokenize{document assets/images/a b.png}" in latex_document
    assert latex_document == render_module.render_latex(
        image_middle, asset_base_path="document assets", document_title="Unified Render"
    )
    ...
    middle = _middle(
        _page(
            ...
            ImageBlock(
                type="image",
                index=3,
                content=[ImageBodyBlock(type="image_body", index=3, content="description", image_path="images/picture.png")],
            ),
        )
    )
    docx = render(middle, RenderFormat.DOCX, options=DocxRenderOptions(asset_resolver=resolve_asset))
...
def test_unified_render_rejects_legacy_format_and_mismatched_options() -> None:
    ...
    with pytest.raises(TypeError, match="RenderFormat"):
        render(middle, "markdown")
    with pytest.raises(TypeError, match="MarkdownRenderOptions"):
        render(middle, RenderFormat.MARKDOWN, options=HtmlRenderOptions())
    with pytest.raises(TypeError, match="StructuredContentRenderOptions"):
        render(middle, RenderFormat.STRUCTURED_CONTENT, options=DocxRenderOptions())
    with pytest.raises(TypeError, match="LatexRenderOptions"):
        render(middle, RenderFormat.LATEX, options=HtmlRenderOptions())
    with pytest.raises(TypeError, match="EpubRenderOptions"):
        render(middle, RenderFormat.EPUB, options=DocxRenderOptions())
    with pytest.raises(TypeError, match="PdfRenderOptions"):
        render(middle, RenderFormat.PDF, options=DocxRenderOptions())
...
def test_public_options_validate_fields() -> None:
    ...
    with pytest.raises(TypeError, match="asset_resolver"):
        EpubRenderOptions(asset_resolver="images")
    with pytest.raises(TypeError, match="asset_resolver"):
        PdfRenderOptions(asset_resolver="images")
    with pytest.raises(TypeError, match="document_title"):
        PdfRenderOptions(document_title=1)
    with pytest.raises(TypeError, match="asset_base_path"):
        LatexRenderOptions(asset_base_path=1)
    with pytest.raises(TypeError, match="document_title"):
        LatexRenderOptions(document_title=1)
