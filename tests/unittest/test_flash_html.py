...
from docvortex.schema import BlockBase, BlockType, CodeBlock, CodeBodyBlock, ImageBlock, ImageBodyBlock, MiddleJson
            ...
            "pages": [
                {
                    ...
                    "blocks": [
                        ...
                        },
                        {
                            "type": "text",
                            "index": 2,
                            "content": [*inline("Text & value < 3 "), equation("x+1"), *inline(" <eq>literal</eq>")],
                        },
                        {"type": "ref_text", "index": 3, "content": inline("Reference text")},
                        {
                            ...
                            "content": [
                                ...
                                {"type": "list", "content": [{"type": "text", "content": inline("- Nested item")}]},
                            ],
                        },
                        {
                            "type": "index",
                            ...
                            "content": [
                                ...
                                {"type": "index", "content": [{"type": "text", "content": inline("Unlinked entry")}]},
                            ],
                        },
                        {
                            "type": "image",
                            ...
                            "sub_type": "diagram",
                            "content": [
                                {"type": "image_body", "index": 6, "content": "Image body", "image_base64": _PNG_URI},
                        ...
                        },
                        {
                            ...
                            "index": 9,
                            "content": [
                                {"type": "table_body", "index": 9, "content": "<table><tr><td>A</td></tr></table>"},
                        ...
                        },
                        {
                            ...
                            "sub_type": "algorithm",
                            "content": [
                                {"type": "algorithm_body", "index": 18, "content": [*inline("if x < y:\n  z="), equation("a")]},
                        ...
                        {"type": "page_footnote", "index": 22, "anchor": "note-anchor", "content": inline("Page footnote")},
...
def test_html_auto_selection_preserves_ancestor_text_styles() -> None:
    ...
    _, model = analyze_native_test_document(payload, file_suffix="html")
...
def test_html_auto_selection_precomputes_repeated_candidate_groups(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    analyze_native_test_document(f"<html><body>{items}</body></html>".encode(), file_suffix="html")
...
def test_html_auto_selection_precomputes_nested_subtree_penalties(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    analyze_native_test_document(f"<html><body>{nested_start}{leaves}{nested_end}</body></html>".encode(), file_suffix="html")
...
def test_html_referenced_external_footnote_keeps_anchor_and_content() -> None:
    ...
    middle, _ = analyze_native_test_document(payload, file_suffix="html")
...
def test_html_auto_selection_appends_notes_for_all_same_document_url_forms(href: str) -> None:
    ...
    middle = analyze_native_test_document(payload, file_suffix="html", source_context=context)[0]
    markdown = render_markdown(middle)
    footnote = next(block for block in middle.pages[0].blocks if block.type == BlockType.PAGE_FOOTNOTE)
...
def test_html_same_document_note_resolution_honors_base_href(base_href: str, expected_note: bool) -> None:
    ...
    markdown = render_markdown(analyze_native_test_document(payload, file_suffix="html", source_context=context)[0])
...
def test_html_local_self_url_appends_referenced_note(tmp_path: Path) -> None:
    ...
    markdown = render_markdown(parse(source).middle_json)
...
def test_html_local_base_images_styles_and_escape_are_bounded(tmp_path: Path) -> None:
    ...
    exported = result.export(tmp_path / "export" / "document.html", output_format="html")
    assert len(exported.asset_paths) == 1
    assert exported.asset_paths[0].read_bytes() == image_path.read_bytes()
    assert _image_body(result.middle_json).image_base64 is None
    assert _image_body(result.middle_json).image_path == image_key
...
def test_html_remote_source_resolves_relative_links_without_fetching_images() -> None:
    ...
    middle, _ = analyze_native_test_document(payload, file_suffix="html", source_context=context)
    markdown = render_markdown(middle)
...
def test_html_comments_count_toward_dom_node_budget(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(HtmlResourceLimitError, match="max_html_nodes"):
        analyze_native_test_document(b"<html><body><!--1--><!--2--><!--3--></body></html>", file_suffix="html")
...
def test_html_inline_stylesheets_enforce_resource_budgets(
    ...
    with pytest.raises(HtmlResourceLimitError, match=expected_limit):
        analyze_native_test_document(payload, file_suffix="html")
...
def test_html_inline_and_local_stylesheets_share_total_budget(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(HtmlResourceLimitError, match="max_html_stylesheet_total_bytes"):
        analyze_native_test_document(payload, file_suffix="html", source_context=context)
...
def test_html_versioned_wire_roundtrips_empty_code_body() -> None:
    ...
    roundtrip = analyze_native_test_document(rendered.encode(), file_suffix="html")[0]
...
def test_html_versioned_wire_roundtrips_all_semantic_types() -> None:
    ...
    default_middle = analyze_native_test_document(default_html.encode(), file_suffix="html")[0]
    full_middle = analyze_native_test_document(full_html.encode(), file_suffix="html")[0]
    standalone_middle = analyze_native_test_document(standalone_html.encode(), file_suffix="html")[0]
...
def test_html_versioned_wire_preserves_visual_rich_content(
    ...
    middle, model = analyze_native_test_document(rendered.encode(), file_suffix="html")
    ...
    normalized_middle = analyze_native_test_document(normalized.encode(), file_suffix="html")[0]
...
def test_html_versioned_wire_roundtrips_canonical_visual_body_variants() -> None:
                    ...
                    "blocks": [
                        {
                            ...
                            "index": 0,
                            "sub_type": "flowchart",
                            "content": [{"type": "image_body", "index": 0, "content": "```mermaid\ngraph TD\nA-->B\n```"}],
                        },
                        {
                            "type": "image",
                            "index": 1,
                        ...
                        {"type": "table", "index": 2, "content": [{"type": "table_body", "index": 2, "content": "A  B\n1  2"}]},
                        {
                            "type": "table",
                            "index": 3,
                            "content": [
                                {"type": "table_body", "index": 3, "content": "", "image_url": "https://example.com/table.png"}
                            ],
                        },
                        {"type": "table", "index": 4, "content": [{"type": "table_body", "index": 4, "content": ""}]},
                    ],
                }
            ],
            "is_full_document": True,
    ...
    middle, _ = analyze_native_test_document(rendered.encode(), file_suffix="html")
...
def test_html_versioned_wire_distinguishes_index_carrier_from_inline_link() -> None:
                    ...
                    "blocks": [
                        {
                            "type": "index",
                            "index": 0,
                            "content": [{"type": "text", "content": [hyperlink("https://example.com/docs", "External")]}],
                        }
                    ],
                }
            ],
    ...
    decode_result = decode_docvortex_html_wire(document.body, HtmlResourceContext(document.source_context))
    middle = analyze_native_test_document(rendered.encode(), file_suffix="html")[0]
...
def test_html_noncanonical_wire_structural_edits_use_generic_fallback(edit_kind: str) -> None:
    ...
    decode_result = decode_docvortex_html_wire(document.body, HtmlResourceContext(document.source_context))
    middle, model = analyze_native_test_document(str(soup).encode(), file_suffix="html")
            ...
            "pages": [
                {
                    "page_idx": 0,
                    "blocks": [
                        {"type": "list", "index": 0, "content": [{"type": "text", "index": 0, "content": inline("- Original")}]}
                    ],
                }
            ],
            "is_full_document": True,
...
def test_html_invalid_versioned_markers_fallback_without_partial_results() -> None:
            ...
            "pages": [
                {
                    "page_idx": 0,
                    "blocks": [
                        {"type": "text", "index": 0, "content": [*inline("WIRESENTINEL "), equation("WIREFORMULA")]},
                        {
                            "type": "image",
                            "index": 1,
                            "content": [
    ...
    for variant in variants:
        markdown = render_markdown(analyze_native_test_document(variant.encode(), file_suffix="html")[0])
...
def test_html_versioned_wire_multiple_owned_images_fall_back_without_loss(
    ...
    _, model = analyze_native_test_document(str(soup).encode(), file_suffix="html")
    image_urls = [str(block["image_url"]) for block in model.pages[0] if block.get("image_url")]
...
def test_html_versioned_wire_visible_sibling_falls_back_without_loss(position: str) -> None:
    ...
    markdown = render_markdown(analyze_native_test_document(str(soup).encode(), file_suffix="html")[0])
    ...
    source = MiddleJson.model_validate(
        {
            "pages": [
                {
                    "page_idx": 0,
                    "blocks": [{"type": "page_footnote", "index": 0, "anchor": "note", "content": inline("Original note")}],
                }
            ],
            "is_full_document": True,
            "metadata": {"file_suffix": "html", "producer": {"name": "docvortex", "version": "0.2.0"}},
                    ...
                    "blocks": [
                        {
                            ...
                            "sub_type": "algorithm",
                            "content": [
                                {"type": "algorithm_body", "index": 0, "content": [*inline("Step A "), equation("x+1")]}
                            ],
                        }
                    ],
                }
                    ...
                    "blocks": [
                        {
                            "type": "table",
                            "index": 0,
                            "content": [{"type": "table_body", "index": 0, "content": "<table><tr><td>A</td></tr></table>"}],
                        }
                    ],
                }
            ],
...
def test_html_marker_fallback_does_not_double_resolve_images(monkeypatch: pytest.MonkeyPatch) -> None:
            ...
            "pages": [
                {
                    ...
                    "blocks": [
                        {"type": "text", "index": 0, "content": inline("Fallback")},
                        {
                            ...
                            "index": 1,
                            "content": [
                                {"type": "image_body", "index": 1, "content": "", "image_url": "https://example.com/once.png"}
                            ],
                        },
                    ],
                }
    ...
    middle = analyze_native_test_document(str(soup).encode(), file_suffix="html")[0]
