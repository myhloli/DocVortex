...
from docvortex.analyzers.native import DocxModel, PptxModel, XlsxModel
from docvortex.analyzers.native.office.docx.docx_converter import DocxConverter
from docvortex.analyzers.native.office.equation.ooxml import OoxmlEquationDecoder, is_mathtype_equation_prog_id
...
def test_invalid_ooxml_mtef_preview_exports_to_sidecar(
    ...
    middle, _model = analyze_native_test_document(builders[file_suffix]([b"invalid MTEF"]), file_suffix=file_suffix)

    result = export_middle_json(middle, tmp_path / file_suffix)
