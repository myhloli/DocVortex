...
DocVortex 在首次 PDF 访问时安装进程级 PDFium 字体提供器。有效嵌入字体继续
...
# DocVortex 自动完成此步骤。直接使用 pypdfium2 的宿主应在首次字体使用前调用。
...
直接访问 pypdfium2 时仍需使用 `docvortex.document.pdf.pdfium.pdfium_guard()`
...
初始化。DocVortex 自带的渲染进程池已经完成接入及父进程退出监控。
...
`docvortex.document.pdf.font_runtime` 的 DEBUG 日志记录有限数量的委托诊断。
...
字符索引以及 DocVortex 的 `source_indices` 规则不变。缺字环境更换字体后，PDFium
...
完整来源见[字体清单](../src/docvortex/resources/fonts/manifest.json)，
对应[原始 NOTICE](../src/docvortex/resources/fonts/NOTICE) 随 wheel 与 sdist 分发。
